...
function Profile() {
  ...
  function Videos(){
    if(band && band.Portofolios){
      let portofolioVideo = band?.Portofolios.filter(portofolio => portofolio.portofolioType === "video" && !portofolio.fileUrl.includes('youtube'));
      if(portofolioVideo.length > 0){
        return (
            portofolioVideo.map((portofolio) => {
              return (
  ...
  function Audios(){
    if(band && band.Portofolios){
      let portofolioAudio = band?.Portofolios.filter(portofolio => portofolio.portofolioType === "audio");
            ...
            portofolioVideo.map((portofolio) => {
              ...
              return (
                <div className="w-full px-0 md:px-1 mb-4 rounded-lg shadow-lg overflow-hidden">
                  <iframe className="w-full h-96" src={`https://www.youtube.com/embed/` + portofolio.fileUrl.match(ID_REGEX)[1]} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
              );
            })
        )
        ...
        <div className="flex items-start w-full md:w-1/3">
          <div className="bg-white overflow-hidden shadow rounded-lg sticky top-4">
            <div className="px-4 py-5 sm:p-6">
              ...
              <Link to={loginStatus ? "/order/"+ band?.id : "/login"}
              ...
              <div class="">
                <nav class="-mb-px flex" aria-label="Tabs">
                  <span
                    ...
                    className={ (showType === "video" ? 'text-indigo-600' : 'text-gray-500') + ' border-transparent hover:text-indigo-700 hover:border-indigio-300 w-1/3 py-4 px-1 text-center border-b-2 font-medium text-sm'}
                  >
                    Video
                  </span>
                  <span
                    onClick={() => { setShowType('audio') }}
                    className={ (showType === "audio" ? 'text-indigo-600' : 'text-gray-500') + ' border-transparent hover:text-indigo-700 hover:border-indigio-300 w-1/3 py-4 px-1 text-center border-b-2 font-medium text-sm'}
                  >
                    Audio
                  </span>
                  <span
                    onClick={() => { setShowType('youtube') }}
                    className={ (showType === "youtube" ? 'text-indigo-600' : 'text-gray-500') + ' border-transparent hover:text-indigo-700 hover:border-indigio-300 w-1/3 py-4 px-1 text-center border-b-2 font-medium text-sm'}
                    aria-current="page"
                  >
                    Youtube
                  </span>
